...
# Function to extract content from Oxylabs API response
# what is isinstance ? It is a built-in function in Python that checks if an object is an instance or subclass of a class or a tuple of classes. How ? It takes two arguments: the object to be checked and the class or tuple of classes to check against. It returns True if the object is an instance or subclass of the specified class(es), and False otherwise.
...
# Function to send a POST request to Oxylabs API
...
# Normalize product data to convert it into a readable format
def normalize_product(content):
    category_path = [] 
    # get all the categories that this product is in so that when later on we look for competitors we can match categories

    if content.get("category_path"):
        category_path = [cat.strip() for cat in content["category_path"] if cat]

    # pulling out the information we care about
    return {
